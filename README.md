...
1. 将字体原文件放在项目外的字体库中
2. 生成 `utils/fontData/` 中对应的按需加载字体数据模块
3. 在 `utils/fonts.ts` 的 `PROJECT_FONT_OPTIONS` 中登记字体名称、CSS font-family 和 `embeddedSourceId`
3. 重新运行 TypeScript 检查和生产构建

内置字体通过按需 JS 数据块加载，不暴露 `/fonts` 或 `/assets/fonts` 静态字体 URL，减少 NAS 或反向代理拦截字体文件的概率。`public/fonts` 已配置忽略规则，避免误把完整字体库提交进仓库。完整字体包建议放在项目外，需要试用时用编辑器里的“上传字体”按钮临时加载。
